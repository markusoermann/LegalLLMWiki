...
- 🔎 **Mechanische Prüfwerkzeuge** (`tools/`): citekeys gegen Zotero, `resource:`-URIs, Abgleich der Normknoten gegen die amtliche konsolidierte Fassung, Wächter für gestaffelte Anwendbarkeit
...
5. **Loslegen:** `ingest @citekey`, `query wiki: …`, `lint wiki`, `verify wiki`, `bench wiki` → [`docs/de/05-workflows.md`](docs/de/05-workflows.md)

Juristische Besonderheiten: [`docs/de/06-recht-features.md`](docs/de/06-recht-features.md) · Verifikation & Benchmark: [`docs/de/07-verifikation.md`](docs/de/07-verifikation.md) · Wiki-MCP-Server: [`docs/de/08-mcp-server.md`](docs/de/08-mcp-server.md) · Konzept/Architektur: [`docs/de/01-konzept.md`](docs/de/01-konzept.md)

## Repo-Aufbau

```
docs/        Setup-Guide (01–08), je Sprache: docs/de/ + docs/en/
...
tools/       Mechanische Prüfwerkzeuge (citekeys, resource-URIs, Normtext-Abgleich, Staffelung)
```
...
- **LLM-Wiki-Muster** — Das zugrunde liegende Konzept (ein KI-gepflegtes, „kompoundierendes" Markdown-Wissens-Wiki als Schicht zwischen Nutzer und Rohquellen) stammt von **Andrej Karpathy** ([Gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f)), vorgestellt im April 2026. LegalLLMWiki ist eine für juristische Inhalte spezialisierte, agent-agnostische Umsetzung dieses Musters.
...
- **Verifikations-Pass (Paper2Agent)** — Die Belegschicht, der Verifikations-Pass und die Zerlegung des Ingests in Sub-Agenten sind dem *test verifier–improver* und der Orchestrator-Architektur aus **Miao, Davis, Zhang, Pritchard & Zou, „Reimagining research papers as interactive and reliable AI agents", Nature (2026), DOI [10.1038/s41586-026-11044-y](https://doi.org/10.1038/s41586-026-11044-y)** nachgebildet; Referenzimplementierung [jmiao24/Paper2Agent](https://github.com/jmiao24/Paper2Agent). Übernommen wurde das Prinzip, dass kein Artefakt ohne Prüfung gegen seine Quelle in den fertigen Bestand gelangt, bewusst nicht die ausführbare Tool-Schicht: juristische Inhalte haben keinen maschinell prüfbaren Wahrheitsmaßstab, geprüft wird deshalb Belegtheit, nicht Richtigkeit.
- **LKIF-Core (juristisches Vokabular)** — Die kontrollierten Vokabulare für die Normfunktion (`normtyp`, Hohfeld-granular: Deontik + Hohfeld'sche Rechtspositionen + Funktionsnormen), die Trennung von Inkrafttreten und Wirksamkeit (`in_kraft`/`wirksam_ab`), die Bindungswirkung von Leitentscheidungen (`bindungswirkung`) sowie die typisierten Modifikations-Relationen (`setzt aus`/`erklärt für nichtig`/`wirkt nach`/`wirkt zurück`) sind an die **LKIF-Core-Rechtsontologie** angelehnt (Module `norm` und `time-modification`). Übernommen wurde bewusst nur das Vokabular, nicht die formale OWL-/SPARQL-Architektur — der Ansatz bleibt Obsidian-nativ. Referenz: [RinkeHoekstra/lkif-core](https://github.com/RinkeHoekstra/lkif-core).
